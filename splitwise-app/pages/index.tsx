...
function buildEqualParticipants(users: User[], activeIds: string[]) {
  const uniqueActiveIds = activeIds.filter((id, index) => activeIds.indexOf(id) === index);
  if (!users.length) return [] as Participant[];
  if (!uniqueActiveIds.length) return users.map((user) => ({ userId: user.id, percent: 0 }));

  const basePercent = Math.floor((100 / uniqueActiveIds.length) * 10000) / 10000;
  let assigned = 0;

  return users.map((user) => {
    if (!uniqueActiveIds.includes(user.id)) {
      return { userId: user.id, percent: 0 };
    }

    const activeIndex = uniqueActiveIds.indexOf(user.id);
    const isLastActive = activeIndex === uniqueActiveIds.length - 1;
...
function buildDebtPairs(expenses: Expense[]) {
  ...
  return Object.entries(pairMap)
    .map(([pairId, pair]) => {
      ...
      const settledAmount = pair.settledLines.reduce((sum, line) => sum + line.amount, 0);
...
function ParticipantEditor({ users, participants, setParticipants, amount }: {
  ...
  const initialInactiveIds = users
    .filter((user) => (participantMap.get(user.id)?.percent ?? 0) <= 0)
    .map((user) => user.id);

  const [inactiveIds, setInactiveIds] = useState<string[]>(initialInactiveIds);
  const [inputValues, setInputValues] = useState<Record<string, string>>(() => Object.fromEntries(
    users.map((user) => {
      const percent = participantMap.get(user.id)?.percent ?? 0;
      return [user.id, formatPercentInput(percent)];
    })
  ));

  useEffect(() => {
    const nextInactiveIds = users
      .filter((user) => (participants.find((participant) => participant.userId === user.id)?.percent ?? 0) <= 0)
      .map((user) => user.id);

    setInactiveIds(nextInactiveIds);
    setInputValues((current) => {
      const nextValues: Record<string, string> = { ...current };
      users.forEach((user) => {
        const percent = participants.find((participant) => participant.userId === user.id)?.percent ?? 0;
        if (!nextInactiveIds.includes(user.id)) {
          nextValues[user.id] = formatPercentInput(percent);
        } else if (!(user.id in nextValues)) {
          nextValues[user.id] = formatPercentInput(percent);
        }
      });
      return nextValues;
  ...
  const activeIds = users.filter((user) => !inactiveIds.includes(user.id)).map((user) => user.id);
  ...
  const syncInputValues = (nextParticipants: Participant[], preservedInactiveIds?: string[]) => {
    const inactiveSet = new Set(preservedInactiveIds ?? inactiveIds);

    setInputValues((current) => {
      const nextValues: Record<string, string> = { ...current };
      nextParticipants.forEach((participant) => {
        if (!inactiveSet.has(participant.userId)) {
          nextValues[participant.userId] = formatPercentInput(participant.percent);
        }
      });
      return nextValues;
    });
  ...
  const applyEqualDistribution = (nextActiveIds: string[]) => {
    const nextParticipants = buildEqualParticipants(users, nextActiveIds);
    setParticipants(nextParticipants);
    syncInputValues(nextParticipants, users.filter((user) => !nextActiveIds.includes(user.id)).map((user) => user.id));
  };

  const toggleUser = (userId: string) => {
    const isInactive = inactiveIds.includes(userId);
    if (!isInactive && activeIds.length === 1) return;

    const nextInactiveIds = isInactive
      ? inactiveIds.filter((id) => id !== userId)
      : [...inactiveIds, userId];

    const nextActiveIds = users
      .map((user) => user.id)
      .filter((id) => !nextInactiveIds.includes(id));

    setInactiveIds(nextInactiveIds);
    applyEqualDistribution(nextActiveIds);
  };

  const updatePercent = (userId: string, rawValue: string) => {
    setInputValues((current) => ({ ...current, [userId]: rawValue }));
    ...
    const normalized = Math.min(Math.max(parsed, 0), 100);
    setParticipants(
      users.map((user) => ({
        userId: user.id,
        percent: user.id === userId
          ? normalized
          : participants.find((participant) => participant.userId === user.id)?.percent ?? 0,
      }))
    );
  };

  const normalizePercentField = (userId: string) => {
    const participant = participants.find((entry) => entry.userId === userId);
    const fallback = participant?.percent ?? 0;
    setInputValues((current) => ({ ...current, [userId]: formatPercentInput(fallback) }));
  };

  const distribute = () => {
    applyEqualDistribution(activeIds);
  };
    ...
    <div>
      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', gap: 12, marginBottom: 14 }}>
        ...
        <div style={{ display: 'flex', alignItems: 'center', gap: 10, flexWrap: 'wrap', justifyContent: 'flex-end' }}>
          ...
          <Btn variant="ghost" size="sm" onClick={distribute}>
      ...
      <div style={{ display: 'flex', flexDirection: 'column', gap: 10 }}>
        {users.map((user) => {
          const participant = participants.find((entry) => entry.userId === user.id) ?? { userId: user.id, percent: 0 };
          const isInactive = inactiveIds.includes(user.id);
          const displayValue = inputValues[user.id] ?? formatPercentInput(participant.percent);
          const shareAmount = Number.isFinite(amountNumber)
            ? ((amountNumber || 0) * participant.percent) / 100
            : 0;
            ...
            <div key={user.id} style={{ display: 'flex', alignItems: 'center', gap: 10 }}>
              <button
                ...
                style={{
                  ...
                  height: 22,
                  borderRadius: 6,
                  border: `2px solid ${isInactive ? 'var(--border)' : 'var(--accent)'}`,
                  background: isInactive ? 'transparent' : 'var(--accent)',
                  cursor: 'pointer',
                  display: 'flex',
                  alignItems: 'center',
              ...
              >
                {!isInactive && <span style={{ color: '#08110d', fontSize: 12, fontWeight: 800 }}>✓</span>}
              </button>

              <span style={{ flex: 1, fontSize: 14, color: isInactive ? 'var(--muted)' : 'var(--text)' }}>
                {user.name}
              </span>

              <div style={{ display: 'flex', alignItems: 'center', gap: 6, opacity: isInactive ? 0.45 : 1 }}>
                <input
                  type="text"
                  inputMode="decimal"
                  value={displayValue}
                  ...
                  onChange={(e) => updatePercent(user.id, e.target.value)}
                  onBlur={() => normalizePercentField(user.id)}
                  style={{
                    width: 72,
                    background: isInactive ? 'rgba(255,255,255,0.02)' : 'var(--surface2)',
                    border: '1px solid var(--border)',
                    borderRadius: 8,
                    color: isInactive ? 'var(--muted)' : 'var(--text)',
                    padding: '6px 8px',
                    fontSize: 16,
                    outline: 'none',
                    textAlign: 'right',
...
function AddExpenseTab({ users, currentUser, token, onAdded }: {
  ...
  const [participants, setParticipants] = useState<Participant[]>(() =>
    buildEqualParticipants(users, users.map((user) => user.id))
  );
  const [error, setError] = useState('');
  const [loading, setLoading] = useState(false);
  ...
  const handleSubmit = async () => {
    ...
    setParticipants(buildEqualParticipants(users, users.map((user) => user.id)));
    setSuccess(true);
...
function DebtDetailModal({ pair, users, currentUser, token, onClose, onRefresh }: {
  ...
  const getName = (id: string) => users.find((user) => user.id === id)?.name || id;
  const creditorCanSettle = (line: DebtLine) => line.to === currentUser.id;

  const handleToggleDebt = async (line: DebtLine) => {
    setLoadingKey(line.key);
    ...
    >
      ...
      >
          ...
          <div>
            ...
            {pair.netFrom && pair.netTo ? (
              <p style={{ fontSize: 14, color: 'var(--muted)', marginTop: 4 }}>
                {getName(pair.netFrom)} schuldet {getName(pair.netTo)} aktuell <span style={{ color: 'var(--text)', fontWeight: 700 }}>{fmt(pair.netAmount)}</span>
              </p>
            ) : (
        ...
        <Card style={{ marginBottom: 16, padding: 16 }}>
          {showNetting && pair.netFrom && pair.netTo ? (
            <div style={{ display: 'flex', flexDirection: 'column', gap: 8 }}>
              ...
              >
                <span style={{ color: 'var(--text)', fontWeight: 700 }}>
                  Nach Verrechnung bleibt offen
                </span>
                <span style={{ fontFamily: 'DM Mono', fontWeight: 700, color: 'var(--accent)' }}>
                  {getName(pair.netFrom)} → {getName(pair.netTo)} · {fmt(pair.netAmount)}
                </span>
              </div>
            </div>
          ) : pair.netFrom && pair.netTo ? (
            ...
            <p style={{ color: 'var(--muted)', fontSize: 14 }}>Keine offene Restschuld.</p>
          )}
        </Card>
        ...
        <div style={{ display: 'flex', flexDirection: 'column', gap: 12 }}>
          <div>
            ...
            {pair.openLines.length === 0 ? (
              <Card style={{ padding: 16 }}>
                <p style={{ color: 'var(--muted)', fontSize: 14 }}>Keine offenen Einzelposten mehr.</p>
              </Card>
            ) : (
              <div style={{ display: 'flex', flexDirection: 'column', gap: 8 }}>
                {pair.openLines.map((line) => (
                  <Card key={line.key} style={{ padding: 14 }}>
                    <div style={{ display: 'flex', justifyContent: 'space-between', gap: 12, alignItems: 'flex-start' }}>
                      <div style={{ flex: 1 }}>
                        <p style={{ fontWeight: 700, fontSize: 14 }}>{line.description}</p>
                        <p style={{ fontSize: 13, color: 'var(--muted)', marginTop: 4 }}>
                          {getName(line.from)} → {getName(line.to)} · {fmtDate(line.date)}
                        </p>
                      </div>

                      <div style={{ textAlign: 'right', minWidth: 110 }}>
                        <p style={{ fontFamily: 'DM Mono', fontWeight: 700, color: 'var(--text)' }}>
                          {fmt(line.amount)}
                        </p>

                        {creditorCanSettle(line) ? (
                          <Btn
                            size="sm"
                            variant="ghost"
                            onClick={() => handleToggleDebt(line)}
                            disabled={loadingKey === line.key}
                            style={{ marginTop: 8, width: '100%' }}
                          >
                            {loadingKey === line.key ? '…' : 'Beglichen'}
                          </Btn>
                        ) : (
                          <p style={{ fontSize: 12, color: 'var(--muted)', marginTop: 8 }}>
                            Nur {getName(line.to)} kann buchen
                          </p>
                        )}
                      </div>
                    </div>
          ...
          {pair.settledLines.length > 0 && (
            <div>
              ...
              <div style={{ display: 'flex', flexDirection: 'column', gap: 8 }}>
                {pair.settledLines.map((line) => (
                  <Card key={line.key} style={{ padding: 14, opacity: 0.88 }}>
                    <div style={{ display: 'flex', justifyContent: 'space-between', gap: 12, alignItems: 'flex-start' }}>
                      <div style={{ flex: 1 }}>
                        <p style={{ fontWeight: 700, fontSize: 14 }}>{line.description}</p>
                        <p style={{ fontSize: 13, color: 'var(--muted)', marginTop: 4 }}>
                      ...
                      <div style={{ textAlign: 'right', minWidth: 110 }}>
                        <p style={{ fontFamily: 'DM Mono', fontWeight: 700, color: 'var(--accent)' }}>
                          {fmt(line.amount)}
                        </p>
                        ...
                        {creditorCanSettle(line) ? (
                          <Btn
                            ...
                            style={{ marginTop: 8, width: '100%' }}
                          >
                            {loadingKey === line.key ? '…' : 'Wieder offen'}
                          </Btn>
                        ) : (
                          <p style={{ fontSize: 12, color: 'var(--muted)', marginTop: 8 }}>Schon verbucht</p>
                        )}
                      </div>
                    </div>
                  </Card>
