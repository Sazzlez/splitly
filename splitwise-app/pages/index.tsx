...
interface DebtPair {
  ...
  settledNetFrom: string | null;
  settledNetTo: string | null;
  status: 'open' | 'settled' | 'balanced';
}
...
// ─── Helpers ──────────────────────────────────────────────────────────────────
const fmt = (n: number) => round2(n).toFixed(2).replace('.', ',') + ' €';
...
function formatPercentInput(value: number) {
  ...
  const asString = Number.isInteger(rounded) ? String(rounded) : String(rounded).replace(/\.?0+$/, '');
  return asString.replace('.', ',');
}

function buildEqualParticipants(users: User[], activeIds?: string[]) {
  const selectedIds = (activeIds ?? users.map((user) => user.id)).filter((id, index, array) => array.indexOf(id) === index);
  const enabledIds = selectedIds.length ? selectedIds : users.map((user) => user.id);

  if (!users.length) return [] as Participant[];

  let assigned = 0;
  return users.map((user) => {
    if (!enabledIds.includes(user.id)) return { userId: user.id, percent: 0 };

    const enabledIndex = enabledIds.indexOf(user.id);
    const isLastEnabled = enabledIndex === enabledIds.length - 1;
    const percent = isLastEnabled
      ? round4(100 - assigned)
      : round4(100 / enabledIds.length);

    assigned = round4(assigned + percent);
    return { userId: user.id, percent };
  });
}

function getExpenseShare(expense: Expense, debtorUserId: string) {
  const participant = expense.participants.find((entry) => entry.userId === debtorUserId);
  if (!participant) return 0;
  return round2((expense.amount * participant.percent) / 100);
}
...
function buildDebtPairs(expenses: Expense[]) {
  ...
  expenses.forEach((expense) => {
    expense.participants.forEach((participant) => {
      ...
      const totalAmount = getExpenseShare(expense, participant.userId);
      if (totalAmount <= EPSILON) return;

      const legacySettledAmount = expense.settledBy.includes(participant.userId) ? totalAmount : 0;
      const settledAmount = round2(clamp(expense.settledAmounts?.[participant.userId] ?? legacySettledAmount, 0, totalAmount));
      const openAmount = round2(totalAmount - settledAmount);
  ...
  return Object.entries(pairMap)
    .map(([pairId, pair]) => {
      const openAToB = round2(pair.lines
        .filter((line) => line.from === pair.userA && line.to === pair.userB)
        .reduce((sum, line) => sum + line.openAmount, 0));

      const openBToA = round2(pair.lines
        .filter((line) => line.from === pair.userB && line.to === pair.userA)
        .reduce((sum, line) => sum + line.openAmount, 0));

      const settledAToB = round2(pair.lines
        .filter((line) => line.from === pair.userA && line.to === pair.userB)
        .reduce((sum, line) => sum + line.settledAmount, 0));

      const settledBToA = round2(pair.lines
        .filter((line) => line.from === pair.userB && line.to === pair.userA)
        .reduce((sum, line) => sum + line.settledAmount, 0));

      const deltaOpen = round2(openAToB - openBToA);
      const deltaSettled = round2(settledAToB - settledBToA);

      const netAmount = Math.abs(deltaOpen) > EPSILON ? round2(Math.abs(deltaOpen)) : 0;
      const netFrom = deltaOpen > EPSILON ? pair.userA : deltaOpen < -EPSILON ? pair.userB : null;
      const netTo = deltaOpen > EPSILON ? pair.userB : deltaOpen < -EPSILON ? pair.userA : null;

      const settledNetAmount = Math.abs(deltaSettled) > EPSILON ? round2(Math.abs(deltaSettled)) : 0;
      const settledNetFrom = deltaSettled > EPSILON ? pair.userA : deltaSettled < -EPSILON ? pair.userB : null;
      const settledNetTo = deltaSettled > EPSILON ? pair.userB : deltaSettled < -EPSILON ? pair.userA : null;

      const status = netAmount > EPSILON ? 'open' : settledNetAmount > EPSILON ? 'settled' : 'balanced';
      ...
      return {
        ...
        lines: [...pair.lines].sort((a, b) => new Date(a.date).getTime() - new Date(b.date).getTime()),
        openAToB,
        openBToA,
        settledAToB,
        settledBToA,
    ...
    .filter((pair) => pair.status !== 'balanced')
    .sort((a, b) => {
      if (a.status !== b.status) return a.status === 'open' ? -1 : 1;
      const amountA = a.status === 'open' ? a.netAmount : a.settledNetAmount;
      const amountB = b.status === 'open' ? b.netAmount : b.settledNetAmount;
      if (Math.abs(amountB - amountA) > EPSILON) return amountB - amountA;
      return new Date(b.latestDate).getTime() - new Date(a.latestDate).getTime();
    });
}

function buildSettlementPlan(pair: DebtPair, action: 'settle' | 'unsettle') {
  const isSettling = action === 'settle';
  const targetAmount = isSettling ? pair.netAmount : pair.settledNetAmount;
  const sourceUserId = isSettling ? pair.netFrom : pair.settledNetFrom;
  const targetUserId = isSettling ? pair.netTo : pair.settledNetTo;

  if (!sourceUserId || !targetUserId || targetAmount <= EPSILON) return [] as SettlementPlanEntry[];

  let remaining = round2(targetAmount);
  const candidates = pair.lines
    .filter((line) => line.from === sourceUserId && line.to === targetUserId)
    .filter((line) => isSettling ? line.openAmount > EPSILON : line.settledAmount > EPSILON)
    .sort((a, b) => isSettling
  ...
  for (const line of candidates) {
    ...
    const available = isSettling ? line.openAmount : line.settledAmount;
    const stepAmount = round2(Math.min(available, remaining));
    if (stepAmount <= EPSILON) continue;
    ...
    remaining = round2(remaining - stepAmount);
  }

  return remaining <= 0.01 ? plan : [];
}
...
function ParticipantEditor({ users, participants, setParticipants, amount }: {
  ...
  useEffect(() => {
    ...
    participants.forEach((participant) => {
      nextDrafts[participant.userId] = formatPercentInput(participant.percent);
    });
  ...
  const toggleUser = (userId: string) => {
    const activeIds = participants.filter((participant) => participant.percent > 0).map((participant) => participant.userId);
  ...
  const commitPercent = (userId: string, rawValue: string) => {
    if (!rawValue.trim()) {
      setDrafts((current) => ({
        ...current,
        [userId]: formatPercentInput(participants.find((participant) => participant.userId === userId)?.percent ?? 0),
      }));
      return;
    }
    ...
    const clamped = round4(clamp(parsed, 0, 100));

    setParticipants(
      participants.map((participant) => (
        participant.userId === userId ? { ...participant, percent: clamped } : participant
      ))
    );

    setDrafts((current) => ({
      ...current,
      [userId]: formatPercentInput(clamped),
    }));
  };
  ...
  const rebalanceRemainder = () => {
    const active = participants.filter((participant) => participant.percent > 0);
    if (!active.length) return;

    const allButLast = active.slice(0, -1);
    const sumWithoutLast = allButLast.reduce((sum, participant) => sum + participant.percent, 0);
    const remainder = round4(clamp(100 - sumWithoutLast, 0, 100));
    const lastUserId = active[active.length - 1].userId;

    setParticipants(
      participants.map((participant) => (
        participant.userId === lastUserId ? { ...participant, percent: remainder } : participant
      ))
    );
  };
  ...
  return (
    <div>
      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 14, gap: 12 }}>
        <div>
          <span style={{ fontSize: 15, fontWeight: 700 }}>Aufteilen auf</span>
          <p style={{ fontSize: 12, color: 'var(--muted)', marginTop: 4 }}>
            Prozentwerte können jetzt komplett gelöscht und neu eingegeben werden.
          </p>
        </div>
      ...
      <div style={{ display: 'flex', flexDirection: 'column', gap: 10 }}>
        {users.map((user) => {
          ...
          const percentValue = participant?.percent ?? 0;
          const shareAmount = round2((safeAmount * percentValue) / 100);
              ...
              {isActive && (
                <div style={{ display: 'flex', alignItems: 'center', gap: 6 }}>
                  <input
                    type="text"
                    inputMode="decimal"
                    value={drafts[user.id] ?? formatPercentInput(percentValue)}
                    onChange={(e) => {
                      const rawValue = e.target.value.replace(/[^0-9.,]/g, '');
                      setDrafts((current) => ({ ...current, [user.id]: rawValue }));
                      if (rawValue.trim()) {
                        const parsed = parseLocaleNumber(rawValue);
                        if (Number.isFinite(parsed)) {
                          setParticipants(
                            participants.map((currentParticipant) => (
                              currentParticipant.userId === user.id
                                ? { ...currentParticipant, percent: round4(clamp(parsed, 0, 100)) }
                                : currentParticipant
                            ))
                          );
                        }
                    ...
                    onBlur={(e) => commitPercent(user.id, e.target.value)}
                    onKeyDown={(e) => {
                      if (e.key === 'Enter') {
                        commitPercent(user.id, (e.target as HTMLInputElement).value);
...
function AddExpenseTab({ users, currentUser, token, onAdded }: {
  ...
  const handleSubmit = async () => {
    ...
    setParticipants(buildEqualParticipants(users));
    setSuccess(true);
...
function DebtDetailModal({ pair, users, currentUser, token, onClose, onRefresh }: {
  ...
  const getName = (id: string) => users.find((user) => user.id === id)?.name || id;
  const action: 'settle' | 'unsettle' = pair.status === 'open' ? 'settle' : 'unsettle';
  const isSettledState = pair.status === 'settled';
  const actingCreditorId = isSettledState ? pair.settledNetTo : pair.netTo;
  const canManage = actingCreditorId === currentUser.id;
  const plan = buildSettlementPlan(pair, action);

  const handleSubmit = async () => {
    if (!plan.length) {
      setError('Für diese Verrechnung konnten keine gültigen Buchungen erzeugt werden.');
      return;
    }
    ...
    const data = await api('/api/settled', {
      method: 'POST',
      body: JSON.stringify({ action, settlements: plan }),
    }, token);
  ...
  return (
    ...
    >
      ...
      >
        <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 18 }}>
          <div>
            ...
            <p style={{ fontSize: 14, color: 'var(--muted)', marginTop: 4 }}>
              {getName(pair.userA)} und {getName(pair.userB)}
            </p>
          </div>
        ...
        <Card style={{ marginBottom: 16, padding: 16, opacity: isSettledState ? 0.78 : 1 }}>
          <div style={{ display: 'flex', flexDirection: 'column', gap: 10 }}>
            <div style={{ display: 'flex', justifyContent: 'space-between', gap: 12 }}>
              <span style={{ color: 'var(--muted)' }}>{getName(pair.userA)} → {getName(pair.userB)}</span>
              <span style={{ fontFamily: 'DM Mono', fontWeight: 700, color: pair.openAToB > EPSILON ? 'var(--text)' : 'var(--muted)' }}>
                {fmt(pair.openAToB)}
              </span>
            </div>
            ...
            <div style={{ display: 'flex', justifyContent: 'space-between', gap: 12 }}>
              ...
              <span style={{ fontFamily: 'DM Mono', fontWeight: 700, color: pair.openBToA > EPSILON ? 'var(--text)' : 'var(--muted)' }}>
                {fmt(pair.openBToA)}
              </span>
            </div>
            ...
            >
              <div>
                <p style={{ color: 'var(--text)', fontWeight: 700 }}>
                  {isSettledState ? 'Bereits verrechnet' : 'Nach Verrechnung bleibt offen'}
                </p>
                <p style={{ color: 'var(--muted)', fontSize: 13, marginTop: 4 }}>
                  {isSettledState && pair.settledNetFrom && pair.settledNetTo
                    ? `${getName(pair.settledNetFrom)} → ${getName(pair.settledNetTo)}`
                    : pair.netFrom && pair.netTo
                      ? `${getName(pair.netFrom)} → ${getName(pair.netTo)}`
                      : 'Kein offener Restbetrag'}
                </p>
              </div>

              <div style={{ textAlign: 'right' }}>
                <span style={{ fontFamily: 'DM Mono', fontWeight: 700, color: isSettledState ? 'var(--muted)' : 'var(--accent)' }}>
                  {fmt(isSettledState ? pair.settledNetAmount : pair.netAmount)}
                </span>
                {isSettledState && (
        ...
        {canManage ? (
          <Btn
            ...
            variant={isSettledState ? 'ghost' : 'primary'}
          >
            <span>{loading ? '…' : isSettledState ? '↺' : '✓'}</span>
            <span>{isSettledState ? 'Begleichung rückgängig machen' : 'Restschuld als beglichen markieren'}</span>
          </Btn>
        ) : (
          <Card style={{ padding: 14 }}>
            <p style={{ color: 'var(--muted)', fontSize: 14 }}>
              Nur {actingCreditorId ? getName(actingCreditorId) : 'die empfangende Person'} kann diese Verrechnung buchen.
            </p>
          </Card>
        )}
      </div>
...
function DebtsTab({ expenses, users, currentUser, token, onRefresh }: {
  ...
  const myDebts = debtPairs.filter((pair) => pair.status === 'open' && pair.netFrom === currentUser.id);
  const owedToMe = debtPairs.filter((pair) => pair.status === 'open' && pair.netTo === currentUser.id);
  const otherDebts = debtPairs.filter((pair) => pair.status === 'open' && pair.netFrom !== currentUser.id && pair.netTo !== currentUser.id);
  const settledPairs = debtPairs.filter((pair) => pair.status === 'settled');
  ...
  const PairRow = ({ pair, emphasize }: { pair: DebtPair; emphasize: 'danger' | 'accent' | 'neutral' | 'settled' }) => {
    ...
    const directionFrom = isSettled ? pair.settledNetFrom : pair.netFrom;
    const directionTo = isSettled ? pair.settledNetTo : pair.netTo;
    const amount = isSettled ? pair.settledNetAmount : pair.netAmount;
    ...
    return (
      <button
        ...
        style={{
          borderRadius: 12,
          background: 'var(--surface2)',
          border: `1px solid ${isSettled ? 'rgba(110,231,183,0.18)' : 'var(--border)'}`,
          width: '100%',
          textAlign: 'left',
          cursor: 'pointer',
          padding: '14px 16px',
          ...
          <div style={{ flex: 1, minWidth: 0 }}>
            <div style={{ display: 'flex', alignItems: 'center', gap: 6, flexWrap: 'wrap' }}>
              <span style={{ fontSize: 14, fontWeight: 700, color: 'var(--text)' }}>
                {directionFrom ? getName(directionFrom) : getName(pair.userA)}
              </span>
              <span style={{ color: 'var(--muted)', fontSize: 12 }}>→</span>
              <span style={{ fontSize: 14, fontWeight: 700, color: 'var(--text)' }}>
                {directionTo ? getName(directionTo) : getName(pair.userB)}
              </span>
  ...
  return (
    <div style={{ display: 'flex', flexDirection: 'column', gap: 20 }}>
      ...
      <div>
        ...
        <p style={{ fontSize: 13, color: 'var(--muted)' }}>
          Gegenseitige Schulden werden automatisch verrechnet. Beglichene Verrechnungen bleiben grau sichtbar und können wieder geöffnet werden.
        </p>
      </div>
