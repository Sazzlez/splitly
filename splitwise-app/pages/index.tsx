...
// ─── API ──────────────────────────────────────────────────────────────────────
const api = async (path: string, opts?: RequestInit, token?: string): Promise<any> => {
  ...
  try {
    ...
    if (text) {
      ...
      catch { return { error: 'Server-Antwort ungültig' }; }
    } else {
      data = {};
    }
    if (!response.ok && !data?.error) {
      return { error: `Fehler ${response.status}` };
    }
    return data;
  } catch {
    return { error: 'Netzwerkfehler — bitte Verbindung prüfen' };
  }
};
...
function AdminUserManager({ allUsers, currentUser, token }: {
  allUsers: User[];
  currentUser: User;
  token: string;
}) {
  ...
  const [result, setResult] = useState<{ userId: string; userName: string; password: string } | null>(null);
  ...
  const handleSet = async () => {
    ...
    if (pwInput.length < 4) { setError('Passwort muss mindestens 4 Zeichen haben'); return; }
    setLoading(true); setError('');
    const data = await api(
      '/api/admin/reset-password',
      { method: 'PUT', body: JSON.stringify({ userId: activeUserId, newPassword: pwInput }) },
      token,
    );
    setLoading(false);
    if (data?.error) { setError(data.error); return; }
    setResult({ userId: activeUserId, userName: data.userName, password: pwInput });
  };
  ...
  return (
    <div style={{ display: 'flex', flexDirection: 'column', gap: 10 }}>
      <p style={{ fontSize: 13, color: 'var(--muted)', fontWeight: 600, letterSpacing: '0.06em' }}>
      ...
      {result && (
        <div style={{
          ...
          <p style={{ fontSize: 13, fontWeight: 700 }}>
            ✓ Passwort für {result.userName} gesetzt
          </p>
          <div style={{
            background: 'var(--surface2)',
            border: '1px solid var(--border)',
          ...
          <div style={{ display: 'flex', gap: 8 }}>
            ...
            <Btn
              size="sm"
              onClick={() => handleCopy(`Hi ${result.userName}, dein Splitly-Passwort wurde zurückgesetzt: ${result.password}\n\nBitte ändere es nach dem Login im Profil.`)}
              style={{ flex: 1 }}
            >
        ...
        return (
          <div key={u.id} style={{
            ...
            <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
              ...
              {!isActive && (
                <button
                  onClick={() => { setActiveUserId(u.id); setPwInput(''); setError(''); }}
                  style={{
                    background: 'var(--surface)',
                    border: '1px solid var(--border)',
                    borderRadius: 7,
            ...
            {isActive && (
              <div style={{ display: 'flex', flexDirection: 'column', gap: 8 }}>
                ...
                />
                {error && <p style={{ color: 'var(--danger)', fontSize: 12, fontWeight: 600 }}>{error}</p>}
                <div style={{ display: 'flex', gap: 6 }}>
                  <Btn variant="ghost" size="sm" onClick={reset} style={{ flex: 1 }}>Abbrechen</Btn>
                  <Btn size="sm" onClick={handleSet} disabled={loading} style={{ flex: 1 }}>
                    {loading ? '…' : 'Setzen'}
                  </Btn>
                </div>
              </div>
...
// ─── PROFILE PANEL ────────────────────────────────────────────────────────────
function ProfilePanel({ currentUser, allUsers, token, onClose, onLogout, onProfileUpdated }: {
  currentUser: User;
  allUsers: User[];
  token: string;
  onClose: () => void;
  ...
  return (
    <BottomSheet onClose={onClose} title="Profil">
      <div style={{ display: 'flex', flexDirection: 'column', gap: 16 }}>
        ...
        {userIsAdmin && (
          <>
            <AdminUserManager allUsers={allUsers} currentUser={currentUser} token={token} />
            <div style={{ height: 1, background: 'var(--border)' }} />
          </>
        )}
        ...
        {confirmingLogout ? (
          <div style={{
            ...
            <p style={{ fontWeight: 700, fontSize: 14 }}>Wirklich abmelden?</p>
            <div style={{ display: 'flex', gap: 8 }}>
              <Btn variant="ghost" onClick={() => setConfirmingLogout(false)} style={{ flex: 1 }}>Abbrechen</Btn>
              <Btn variant="danger" onClick={onLogout} style={{ flex: 1 }}>Abmelden</Btn>
            </div>
          </div>
        ) : (
          <Btn variant="danger" onClick={() => setConfirmingLogout(true)} style={{ width: '100%' }}>
            Abmelden
          </Btn>
        )}
      </div>
...
// ─── EVENTS PANEL ─────────────────────────────────────────────────────────────
function EventsPanel({ events, allUsers, currentUser, token, onSelectEvent, onClose, onEventsChanged }: {
  events: SplitlyEvent[];
  allUsers: User[];
  currentUser: User;
  token: string;
  ...
  const [kickTarget, setKickTarget] = useState<{ eventId: string; userId: string; name: string } | null>(null);
  ...
  const handleDelete = async (id: string) => {
    setActionLoadingId(id);
    const data = await api('/api/events', { method: 'DELETE', body: JSON.stringify({ id }) }, token);
    setActionLoadingId(null);
    if (data.error) { setError(data.error); return; }
  ...
  const handleKick = async () => {
    ...
    setActionLoadingId(kickTarget.eventId);
    const data = await api(
      '/api/events',
      { method: 'POST', body: JSON.stringify({ action: 'kick', id: kickTarget.eventId, userId: kickTarget.userId }) },
      token,
    );
    setActionLoadingId(null);
    if (data.error) { setError(data.error); setKickTarget(null); return; }
    onEventsChanged(
      events.map((e) =>
        e.id === kickTarget.eventId
          ? { ...e, memberIds: e.memberIds.filter((m) => m !== kickTarget.userId) }
          : e,
      ),
    );
    setKickTarget(null);
    ...
    >
      ...
      <div style={{ maxWidth: 480, margin: '0 auto', width: '100%', padding: '20px 16px', display: 'flex', flexDirection: 'column', gap: 12 }}>
        ...
        {events.map((ev) => {
          ...
          const isBusy = actionLoadingId === ev.id;
            ...
            return (
              <Card key={ev.id}>
                <div style={{ display: 'flex', flexDirection: 'column', gap: 10 }}>
                  ...
                  <div style={{ display: 'flex', gap: 8 }}>
                    <Btn variant="ghost" onClick={() => setRenamingId(null)} disabled={isBusy} style={{ flex: 1 }}>Abbrechen</Btn>
                    <Btn onClick={() => handleRename(ev.id)} disabled={isBusy} style={{ flex: 1 }}>{isBusy ? '…' : 'Speichern'}</Btn>
                  </div>
                </div>
              </Card>
            );
          ...
          if (deletingId === ev.id) {
            return (
              <Card key={ev.id} style={{ borderColor: 'rgba(248,113,113,0.3)' }}>
                <p style={{ fontWeight: 700, marginBottom: 6 }}>Event löschen?</p>
                <p style={{ fontSize: 13, color: 'var(--muted)', marginBottom: 16 }}>
                  „{ev.name}" wird unwiderruflich gelöscht.
                </p>
                <div style={{ display: 'flex', gap: 8 }}>
                  <Btn variant="ghost" onClick={() => setDeletingId(null)} disabled={isBusy} style={{ flex: 1 }}>Abbrechen</Btn>
                  <Btn variant="danger" onClick={() => handleDelete(ev.id)} disabled={isBusy} style={{ flex: 1 }}>{isBusy ? '…' : 'Löschen'}</Btn>
                </div>
              </Card>
            );
          }

          if (kickTarget && kickTarget.eventId === ev.id) {
            return (
              <Card key={ev.id} style={{ borderColor: 'rgba(248,113,113,0.3)' }}>
                <p style={{ fontWeight: 700, marginBottom: 6 }}>{kickTarget.name} entfernen?</p>
                <p style={{ fontSize: 13, color: 'var(--muted)', marginBottom: 16 }}>
                  {kickTarget.name} wird aus „{ev.name}" entfernt und sieht das Event nicht mehr.
                </p>
                <div style={{ display: 'flex', gap: 8 }}>
                  <Btn variant="ghost" onClick={() => setKickTarget(null)} disabled={isBusy} style={{ flex: 1 }}>Abbrechen</Btn>
                  <Btn variant="danger" onClick={handleKick} disabled={isBusy} style={{ flex: 1 }}>{isBusy ? '…' : 'Entfernen'}</Btn>
                </div>
              </Card>
            );
          }
          ...
          return (
            <Card key={ev.id} style={{ padding: 0, overflow: 'hidden' }}>
              ...
              >
                <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', gap: 12 }}>
                  <div style={{ flex: 1, minWidth: 0 }}>
                    <p style={{ fontWeight: 700, fontSize: 16, marginBottom: 6 }}>{ev.name}</p>
                    <div style={{ display: 'flex', alignItems: 'center', gap: 8, flexWrap: 'wrap' }}>
                      <span
                        onClick={(e) => { e.stopPropagation(); handleCopyCode(ev.code, ev.id); }}
                        title="Code kopieren"
                        style={{
                          fontFamily: 'DM Mono',
                          fontSize: 13,
                      ...
                      >
                        {copiedId === ev.id ? '✓ Kopiert' : ev.code}
                      </span>
              ...
              >
                ...
                >
                  {isExpanded ? '▴ Mitglieder' : '▾ Mitglieder'}
                </button>
                {isCreator && (
                  <>
                    <button
                      onClick={() => { setRenamingId(ev.id); setRenameName(ev.name); setError(''); }}
                      style={{
                        background: 'var(--surface2)',
                        border: '1px solid var(--border)',
                        borderRadius: 7,
                    ...
                    <button
                      onClick={() => { setDeletingId(ev.id); setError(''); }}
                      style={{
              ...
              {isExpanded && (
                ...
                >
                  {ev.memberIds.map((mid) => (
                    <div key={mid} style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', fontSize: 13, gap: 10 }}>
                      <span style={{ color: 'var(--text)', flex: 1, minWidth: 0, overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap' }}>
                        {getName(mid)}
                        {mid === currentUser.id && <span style={{ color: 'var(--muted)', fontWeight: 500 }}> · du</span>}
                      </span>
                      <div style={{ display: 'flex', alignItems: 'center', gap: 8, flexShrink: 0 }}>
                        {mid === ev.creatorId && (
                          <span style={{ color: 'var(--muted)', fontSize: 11 }}>Ersteller</span>
                        )}
                        {isCreator && mid !== ev.creatorId && (
                          <button
                            onClick={() => { setKickTarget({ eventId: ev.id, userId: mid, name: getName(mid) }); setError(''); }}
...
// ─── ADD EXPENSE TAB ──────────────────────────────────────────────────────────
function AddExpenseTab({ users, currentUser, onAdd }: {
  users: User[];
  currentUser: User;
  onAdd: (input: { description: string; amount: number; paidBy: string; participants: Participant[] }) => Promise<{ error?: string }>;
}) {
  ...
  // Merge participants when user set changes — preserve user-entered percentages.
  useEffect(() => {
    setParticipants((prev) => {
      const existing = new Map(prev.map((p) => [p.userId, p.percent]));
      const sameSet = prev.length === users.length && users.every((u) => existing.has(u.id));
      if (sameSet) return prev;
      return users.map((u) => ({
        userId: u.id,
        percent: existing.has(u.id) ? (existing.get(u.id) as number) : 0,
      }));
    });
  }, [users]);
...
// ─── EDIT MODAL ───────────────────────────────────────────────────────────────
function EditModal({ expense, users, onClose, onUpdate }: {
  expense: Expense;
  users: User[];
  onClose: () => void;
  onUpdate: (id: string, updates: { description: string; amount: number; paidBy: string; participants: Participant[] }) => Promise<{ error?: string }>;
}) {
...
// ─── ACTIVITY TAB ─────────────────────────────────────────────────────────────
function ActivityTab({ expenses, users, currentUser, onUpdate, onDelete }: {
  expenses: Expense[];
  users: User[];
  currentUser: User;
  onUpdate: (id: string, updates: { description: string; amount: number; paidBy: string; participants: Participant[] }) => Promise<{ error?: string }>;
      ...
      {deletingId && (
        ...
        >
          <Card style={{ maxWidth: 320, width: '100%', textAlign: 'center' }}>
            ...
            {deleteError && (
              <p style={{ color: 'var(--danger)', fontSize: 13, fontWeight: 600, marginBottom: 12 }}>{deleteError}</p>
            )}
            <div style={{ display: 'flex', gap: 10 }}>
              <Btn variant="ghost" onClick={() => { setDeletingId(null); setDeleteError(''); }} style={{ flex: 1 }}>Abbrechen</Btn>
              <Btn
                variant="danger"
...
// ─── DEBT DETAIL MODAL ────────────────────────────────────────────────────────
function DebtDetailModal({ pair, users, currentUser, onClose, onSettle }: {
  pair: DebtPair;
  users: User[];
  currentUser: User;
  onClose: () => void;
  onSettle: (entries: { id: string; debtorUserId: string }[], action: 'settle' | 'unsettle') => Promise<{ error?: string }>;
}) {
  const [loading, setLoading] = useState(false);
  const [error, setError] = useState('');
  ...
  const handleTogglePair = async () => {
    ...
    const result = await onSettle(
      actionEntries.map((line) => ({ id: line.expenseId, debtorUserId: line.from })),
      pair.isSettled ? 'unsettle' : 'settle',
    );
    setLoading(false);
            ...
            confirming ? (
              <div style={{
                ...
                <p style={{ fontWeight: 700, fontSize: 14 }}>
                  {pair.isSettled
                    ? 'Begleichung wirklich rückgängig machen?'
                    : `Schuld in Höhe von ${fmt(pair.displayAmount)} wirklich als beglichen markieren?`}
                </p>
                <div style={{ display: 'flex', gap: 8 }}>
                  <Btn variant="ghost" onClick={() => setConfirming(false)} style={{ flex: 1 }}>Abbrechen</Btn>
                  <Btn
                    onClick={handleTogglePair}
                    disabled={loading}
                    variant={pair.isSettled ? 'danger' : 'primary'}
                    style={{ flex: 1 }}
                  >
                    {loading ? '…' : 'Bestätigen'}
                  </Btn>
                </div>
                {error && <p style={{ color: 'var(--danger)', fontSize: 13, fontWeight: 600, marginTop: 4 }}>{error}</p>}
              </div>
            ) : (
              <Btn
                onClick={() => setConfirming(true)}
                disabled={!actionEntries.length}
                style={{ width: '100%', minHeight: 48, fontSize: 15, fontWeight: 700 }}
                variant={pair.isSettled ? 'ghost' : 'primary'}
              >
                {pair.isSettled ? 'Begleichung rückgängig machen' : 'Schuld als beglichen markieren'}
              </Btn>
            )
          ) : (
            <p style={{ fontSize: 13, color: 'var(--muted)' }}>
...
// ─── DEBTS TAB ────────────────────────────────────────────────────────────────
function DebtsTab({ expenses, users, currentUser, onSettle }: {
  expenses: Expense[];
  users: User[];
  currentUser: User;
  onSettle: (entries: { id: string; debtorUserId: string }[], action: 'settle' | 'unsettle') => Promise<{ error?: string }>;
...
export default function Home() {
  ...
  const loadEvents = useCallback(async (currentToken: string): Promise<SplitlyEvent[]> => {
    try {
  ...
  const loadUsers = useCallback(async (): Promise<void> => {
    try {
      const loadedUsers = await api('/api/users');
  ...
  const loadExpenses = useCallback(async (currentToken: string, eventId: string): Promise<void> => {
    const myRequestId = ++expenseRequestId.current;
    setLoadingData(true);
    try {
      const loadedExpenses = await api(`/api/expenses?eventId=${eventId}`, undefined, currentToken);
      if (myRequestId !== expenseRequestId.current) return; // stale, ignore
      ...
      if (myRequestId === expenseRequestId.current) setLoadingData(false);
    }
  }, []);

  // Initial load: events + users in parallel. Expenses load via separate effect when event is set.
  useEffect(() => {
    if (!token) { setInitializing(false); return; }
    setInitializing(true);
    Promise.all([loadEvents(token), loadUsers()]).finally(() => setInitializing(false));
  }, [token, loadEvents, loadUsers]);

  // Load expenses whenever the active event changes
  useEffect(() => {
    if (token && currentEvent) {
      loadExpenses(token, currentEvent.id);
    } else if (!currentEvent) {
      setExpenses([]);
    }
  }, [token, currentEvent?.id, loadExpenses]);
  ...
  const handleAddExpense = useCallback(async (input: {
    ...
    if (!currentEvent || !token || !currentUser) return { error: 'Kein Event ausgewählt' };
    ...
    const data = await api('/api/expenses', {
      method: 'POST',
      body: JSON.stringify({ ...input, eventId: currentEvent.id }),
    }, token);

    if (data?.error || !data?.id) {
      setExpenses((prev) => prev.filter((e) => e.id !== tempId));
      return { error: data?.error || 'Speichern fehlgeschlagen' };
    }

    setExpenses((prev) => prev.map((e) => (e.id === tempId ? { ...e, id: data.id, date: data.date || e.date } : e)));
    return {};
  }, [currentEvent, token, currentUser]);

  const handleUpdateExpense = useCallback(async (id: string, updates: {
    description: string;
    amount: number;
    paidBy: string;
    participants: Participant[];
    ...
    setExpenses((prev) => {
      prevExpense = prev.find((e) => e.id === id);
    ...
    if (data?.error) {
      if (prevExpense) {
        const captured = prevExpense;
        setExpenses((prev) => prev.map((e) => (e.id === id ? captured : e)));
      }
  ...
  const handleDeleteExpense = useCallback(async (id: string): Promise<{ error?: string }> => {
    if (!token) return { error: 'Nicht angemeldet' };

    let prevExpense: Expense | undefined;
    let prevIndex = -1;
    setExpenses((prev) => {
      prevIndex = prev.findIndex((e) => e.id === id);
      prevExpense = prev[prevIndex];
    ...
    if (data?.error) {
      if (prevExpense) {
        ...
        setExpenses((prev) => {
          const copy = [...prev];
          copy.splice(idx >= 0 ? idx : 0, 0, captured);
          return copy;
        });
      }
      return { error: data.error };
  ...
  const handleSettleExpenses = useCallback(async (
    entries: { id: string; debtorUserId: string }[],
    ...
    setExpenses((prev) => {
      ...
      return prev.map((e) => {
        ...
        if (!entry) return e;
        if (action === 'settle') {
          if (e.settledBy.includes(entry.debtorUserId)) return e;
          return { ...e, settledBy: [...e.settledBy, entry.debtorUserId] };
        }
        return { ...e, settledBy: e.settledBy.filter((uid) => uid !== entry.debtorUserId) };
      });
    });

    const data = await api('/api/settled', { method: 'POST', body: JSON.stringify({ entries }) }, token);
  ...
  const handleSelectEvent = (event: SplitlyEvent) => {
    ...
    setShowEvents(false);
    if (token) loadExpenses(token, event.id);
  };
      ...
      {showEvents && (
        <EventsPanel
          ...
          onEventsChanged={(next) => {
            setEvents(next);
            // If current event was left/deleted, deselect it
            if (currentEvent && !next.some((e) => e.id === currentEvent.id)) {
              setCurrentEvent(null);
              localStorage.removeItem('lastEventId');
            }
          }}
      ...
      {!currentEvent ? (
        ...
        <>
          <div style={{ paddingTop: 72, paddingBottom: 86 }}>
            <div key={currentEvent.id} style={{ maxWidth: 480, margin: '0 auto', padding: '20px 16px' }}>
