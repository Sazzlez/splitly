...
import { getEvents, addEvent, updateEventName, deleteEvent, addUserToEvent, removeUserFromEvent, getEventByCode, ensureEventsSheet } from '@/lib/sheets';
...
export default async function handler(req: NextApiRequest, res: NextApiResponse) {
  ...
  if (req.method === 'POST') {
    const { name, code: joinCode, action, id: targetId, userId: targetUserId } = req.body as {
      name?: string;
      code?: string;
      action?: string;
      id?: string;
    ...
    if (action === 'kick') {
      if (!targetId || !targetUserId) return res.status(400).json({ error: 'Event-ID oder User-ID fehlt' });
      try {
        const events = await getEvents(payload.userId);
        ...
        if (targetUserId === event.creatorId) {
          return res.status(400).json({ error: 'Der Ersteller kann sich nicht selbst entfernen' });
        }
        await removeUserFromEvent(targetId, targetUserId);
        return res.json({ ok: true });
      } catch (error) {
        console.error(error);
        return res.status(500).json({ error: 'Entfernen fehlgeschlagen' });
      }
    }
