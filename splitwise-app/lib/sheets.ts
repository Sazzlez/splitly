...
export async function addUserToEvent(eventId: string, userId: string) {
  ...
  for (let attempt = 0; attempt < 5; attempt++) {
    const memberIds = await readMemberIds(sheets, rowIndex);
    ...
    const verified = await readMemberIds(sheets, rowIndex);
    if (verified.includes(userId)) return;

    await new Promise((r) => setTimeout(r, 100 * (attempt + 1) + Math.random() * 100));
  }
  throw new Error('Konnte Mitglied nicht zum Event hinzufügen (Concurrency)');
}
...
export async function removeUserFromEvent(eventId: string, userId: string) {
  ...
  throw new Error('Konnte Mitglied nicht aus Event entfernen (Concurrency)');
}
